/**
 * @file cfn_svc_test_collection.cpp
...
{
  protected:
    cfn_svc_collection_t     driver{};
    cfn_svc_collection_api_t api{};
...
{
    EXPECT_EQ(cfn_svc_collection_init(nullptr), CFN_HAL_ERROR_BAD_PARAM);
    EXPECT_EQ(cfn_svc_collection_deinit(nullptr), CFN_HAL_ERROR_BAD_PARAM);
}

TEST_F(CollectionTest, WrongPeripheralTypeReturnsBadParam)
{
    driver.base.type = CFN_HAL_PERIPHERAL_TYPE_UNKNOWN;
    EXPECT_EQ(cfn_svc_collection_init(&driver), CFN_HAL_ERROR_BAD_PARAM);
    EXPECT_EQ(cfn_svc_collection_deinit(&driver), CFN_HAL_ERROR_BAD_PARAM);
}
...
TEST_F(CollectionTest, UnimplementedApiReturnsNotSupported)
{
    ...
    EXPECT_EQ(cfn_svc_collection_push_back(&driver, nullptr), CFN_HAL_ERROR_NOT_SUPPORTED);
}
...
TEST_F(CollectionTest, PushPopSizeSuccess)
{
    ...
    EXPECT_EQ(cfn_svc_collection_get_size(&driver, &size), CFN_HAL_ERROR_OK);
    EXPECT_EQ(size, 1);
    EXPECT_EQ(cfn_svc_collection_pop_front(&driver, &output), CFN_HAL_ERROR_OK);
    EXPECT_EQ(output, 42);
}
