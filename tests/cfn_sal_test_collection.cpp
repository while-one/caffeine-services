...
{
  protected:
    cfn_sal_collection_t     driver{};
    cfn_sal_collection_api_t api{};
...
{
    EXPECT_EQ(cfn_sal_collection_init(nullptr), CFN_HAL_ERROR_BAD_PARAM);
    EXPECT_EQ(cfn_sal_collection_deinit(nullptr), CFN_HAL_ERROR_BAD_PARAM);
}

TEST_F(CollectionTest, WrongPeripheralTypeReturnsBadParam)
{
    driver.base.type = CFN_HAL_PERIPHERAL_TYPE_UNKNOWN;
    EXPECT_EQ(cfn_sal_collection_init(&driver), CFN_HAL_ERROR_BAD_PARAM);
    EXPECT_EQ(cfn_sal_collection_deinit(&driver), CFN_HAL_ERROR_BAD_PARAM);
}
...
TEST_F(CollectionTest, UnimplementedApiReturnsNotSupported)
{
    ...
    EXPECT_EQ(cfn_sal_collection_push_back(&driver, nullptr), CFN_HAL_ERROR_NOT_SUPPORTED);
}
...
TEST_F(CollectionTest, OnConfigFailureAbortsInit)
{
    ...
    EXPECT_EQ(cfn_sal_collection_init(&driver), CFN_HAL_ERROR_FAIL);
...
TEST_F(CollectionTest, InitSuccess)
{
    ...
    EXPECT_EQ(cfn_sal_collection_init(&driver), CFN_HAL_ERROR_OK);
...
TEST_F(CollectionTest, PushPopSizeSuccess)
{
    ...
    api.pop_front = [](cfn_sal_collection_t *d, void *item_out) -> cfn_hal_error_code_t
    {
        (void) d;
    ...
    api.get_size = [](cfn_sal_collection_t *d, size_t *s) -> cfn_hal_error_code_t
    {
        (void) d;
        *s = 1;
        return CFN_HAL_ERROR_OK;
    ...
    EXPECT_EQ(size, 1);
    EXPECT_EQ(cfn_sal_collection_pop_front(&driver, &output), CFN_HAL_ERROR_OK);
    EXPECT_EQ(output, 42);
}

TEST_F(CollectionTest, ClearSuccess)
{
    api.clear = [](cfn_sal_collection_t *d) -> cfn_hal_error_code_t
    {
        (void) d;
        return CFN_HAL_ERROR_OK;
    };
    EXPECT_EQ(cfn_sal_collection_clear(&driver), CFN_HAL_ERROR_OK);
}
