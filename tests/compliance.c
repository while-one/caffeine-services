...
static const struct cfn_sal_dev_led_api_s DUMMY_LED_API = {
    .base = {
        .init = dummy_init,
        .deinit = NULL,
...
static const cfn_sal_dev_led_config_t LED_CONFIG = { .active_low = true, .custom = NULL };
...
int main(void)
{
    ...
    cfn_sal_dev_led_t led = { 0 };

    cfn_sal_dev_led_populate(&led, 0xDEAD, NULL, &DUMMY_LED_API, &LED_PHY, &LED_CONFIG, NULL, NULL);
    /* 2. Test Base Initialization (Macro Expansion) */
    cfn_hal_error_code_t err = cfn_sal_dev_led_init(&led);
    ...
    err = cfn_sal_dev_led_toggle(&led);

    /* 4. Test Lock Macro Expansion (CFN_HAL_WITH_LOCK) */
    cfn_hal_error_code_t lock_result;
    CFN_HAL_WITH_LOCK(&led, 100, lock_result, cfn_sal_dev_led_set_state, CFN_SAL_DEV_LED_STATE_ON);
