...
CFN_SVC_CREATE_DRIVER_TYPE(svc_gnss, cfn_svc_gnss_config_t, cfn_svc_gnss_api_t, cfn_svc_phy_t, cfn_svc_gnss_callback_t);

/* Functions inline ------------------------------------------------- */

CFN_HAL_INLINE void cfn_svc_gnss_populate(cfn_svc_gnss_t              *driver,
                                          uint32_t                     peripheral_id,
                                          const cfn_svc_gnss_api_t    *api,
                                          const cfn_svc_phy_t         *phy,
                                          const cfn_svc_gnss_config_t *config,
                                          ...
                                          void                        *user_arg)
{
    ...
    cfn_hal_base_populate(&driver->base, CFN_SVC_TYPE_GNSS, peripheral_id, api ? &api->base : NULL, NULL);
    driver->api = api;
    driver->phy = phy;
...
cfn_hal_error_code_t cfn_svc_gnss_construct(cfn_svc_gnss_t              *driver,
                                            const cfn_svc_gnss_config_t *config,
                                            const cfn_svc_phy_t         *phy,
                                            cfn_svc_gnss_callback_t      callback,
                                            void                        *user_arg);
cfn_hal_error_code_t cfn_svc_gnss_destruct(cfn_svc_gnss_t *driver);
